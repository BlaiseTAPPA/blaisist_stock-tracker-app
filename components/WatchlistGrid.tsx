...
const WatchlistGrid = ({ initialData }: WatchlistGridProps) => {
  ...
  useEffect(() => {
    const cleanedData = initialData.map(stock => ({
      ...stock,
      priceFormatted: stock.priceFormatted ?? "—",
      changeFormatted: stock.changeFormatted ?? "0.00",
    ...
    <div className="grid grid-cols-3 gap-3">
      {Array.from({ length: 12 }).map((_, i) => {
        ...
        return (
          <div
            key={`empty-${i}`}
            className="rounded-lg border border-dashed border-gray-700 bg-gray-800/30 p-3 min-h-[130px]"
          />
        );
      })}
    </div>
